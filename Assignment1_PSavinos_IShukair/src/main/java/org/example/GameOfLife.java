...
public class GameOfLife extends Matrix {
    ...
    public GameOfLife(int rows, int cols) {
        ...
        //add shape to the board. error arrises if out of bounds
    public void addShape(Shape shape, int x, int y){
        int[][] shapeData = shape.getData();
        int[][] gameData = getData();

        if (x + shapeData.length > getRowCount() || y + shapeData[0].length > getColCount[0].length > getColCount())
    ...
    public void step() {
        ...
        //counts live nneighbors of the cell at row, col.
    private int countLiveNeighbors(int[][] data, int row, int col){
            ...
            for (int i = -1; i <= 1; i++) {
                for (int j = -1; j <= 1; j++) {
                    int r = row + i, c = col + j;
                    if (i == 0 && j == 0) continue;
                    if (r >= 0 && r < getRowCount() && c >= 0 && c < getColCount()) count += data[r][c];
                }
            }
        }
        return count;
